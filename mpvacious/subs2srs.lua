...
local config = {
    ...
    audio_format = "mp3", -- opus or mp3
    audio_bitrate = "16k", -- from 16k to 32k
    ...
    deck_name = "French::Movies", -- name of the deck for new cards
    model_name = "French Sentences with Audio", -- Tools -> Manage note types
    sentence_field = "French",
    sentence_field2 = "English",
    audio_field = "Audio",
    image_field = "Note",
    append_media = false, -- True to append video media after existing data, false to insert media before
    disable_gui_browse = true, -- Lets you disable anki browser manipulation by mpvacious.
    ...
    miscinfo_field = "", -- misc notes and source information field
    miscinfo_format = "", -- format string to use for the miscinfo_field, accepts note_tag-style format strings

    -- Forvo support
    use_forvo = "no", -- 'yes', 'no', 'always'
    vocab_field = "French", -- target word field
...
local profiles = {
    profiles = "subs2srs,subs2srs_french_with_Audio,subs2srs_french",
    active = "subs2srs",
}
...
local function copy_sub_to_clipboard_secondary()
    copy_to_clipboard("copy-on-demand", mp.get_property("secondary-sub-text"))
end
...
local function construct_note_fields(sub_text, sub_text2, snapshot_filename, audio_filename)
    local ret = {
        [config.sentence_field] = sub_text,
        [config.sentence_field2] = sub_text2,
...
local filename_factory = (function()
    ...
    local make_audio_filename = function(speech_start, speech_end)
        local filename_timestamp = string.format(
                ...
                -- config.audio_extension -- was removed because .ogg isn't supported in iOS
                '.mp3'
        )
...
local function export_to_anki(gui)
    ...
    local sub2 = mp.get_property("secondary-sub-text")

    local note_fields = construct_note_fields(sub['text'], sub2, snapshot_filename, audio_filename)
...
local main = (function()
    ...
    return function()
        ...
        -- Key bindings
        mp.add_key_binding("e", "mpvacious-export-note", export_to_anki)
        mp.add_key_binding("Ctrl+E", "mpvacious-update-last-note", _ { update_last_note, false })
        mp.add_key_binding("Ctrl+c", "mpvacious-copy-sub-to-clipboard", copy_sub_to_clipboard)
        mp.add_key_binding("Ctrl+C", "mpvacious-copy-sub-to-clipboard-secondary", copy_sub_to_clipboard_secondary)
        mp.add_key_binding("Ctrl+t", "mpvacious-autocopy-toggle", clip_autocopy.toggle)

        -- Open advanced menu
        mp.add_key_binding("Ctrl+a", "mpvacious-menu-open", menu.open)
        ...
        mp.add_key_binding("", "mpvacious-load-next-profile", load_next_profile)
    end
end)()
